...
def mouse_click():
    ...
    if a != state_left:  # Button state changed
        ...
        #print(a)
        if a < 0:
            presses.append(1) #Left Button Pressed
        else:
            presses.append(10) #Left Button Released

    if b != state_right:  # Button state changed
        state_right = b
        #print(b)
        if b < 0:
            presses.append(2) #Right Button Pressed
        else:
            presses.append(20) #Right Button Released
    return presses


'''
while True:
    for i in mouse_click():
'''